...
impl Interpreter {
    pub fn interpret(ast: Vec<AST>) -> Object {
        ...
        for func in ast {
            int.funcs
                .insert(func.unwrap_function().0, FuncBody::AstBody(func));
        }
    ...
    fn walk_function(&mut self, func: FuncBody, args: Vec<Object>) -> Object {
        match func {
            ...
            FuncBody::ClosureBody(function) => function(args),
        }
    }
            ...
            match node {
                ...
                AST::ArrayAssign(arr, pos, val) => {
                    ...
                    match env.get_var(arr.clone()) {
                        Object::WalkedList(mut l) => match walked_pos {
                            Object::Integer(n) => {
    ...
    fn walk_expr(&mut self, expr: Expr, env: &Scope) -> Object {
        match expr {
            ...
            Expr::FuncCall(name, args) => {
                let walked_args = args
                    .into_iter()
                    .map(|arg| self.walk_expr(arg, env))
                    .collect();
            ...
            Expr::ArrayAccess(arr, expr) => {
                ...
                match walked_arr {
                    Object::WalkedList(list) => match walked_expr {
